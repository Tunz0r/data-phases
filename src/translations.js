...
const PERSONAS_EN = [
  ...
  { id: 'hub-eng', label: 'Hub Data & AI Engineer', sublabel: 'Data Foundation + Tech Enabling domains', color: '#10b981', icon: '\u{2699}\u{FE0F}' },
  { id: 'everyday-ai', label: 'Everyday AI Builder', sublabel: 'Non-IT \u2014 builds personal AI tools', color: '#f59e0b', icon: '\u{2728}' },
  { id: 'prod-eng', label: 'Product Team Data & AI Engineer', sublabel: 'IT \u2014 domain AI, product team data & ML', color: '#ec4899', icon: '\u{1F916}' },
]
...
const PERSONAS_DA = [
  ...
  { id: 'hub-eng', label: 'Hub Data & AI Ingeni\u00f8r', sublabel: 'Data Foundation + Tech Enabling dom\u00e6ner', color: '#10b981', icon: '\u{2699}\u{FE0F}' },
  { id: 'everyday-ai', label: 'Hverdags AI-bygger', sublabel: 'Ikke-IT \u2014 bygger personlige AI-v\u00e6rkt\u00f8jer', color: '#f59e0b', icon: '\u{2728}' },
  { id: 'prod-eng', label: 'Produktteam Data & AI Ingeni\u00f8r', sublabel: 'IT \u2014 dom\u00e6ne-AI, produktteam data & ML', color: '#ec4899', icon: '\u{1F916}' },
]
...
const PHASES_EN = [
  {
    ...
    personas: [
      ...
      {
        desc: 'No shared platform exists yet. Data Foundation runs its data warehousing on Azure, while Tech Enabling\u2019s stack is AWS. This cloud split means Data Foundation runs its own infrastructure — Tech Enabling cannot support it yet. Migration from Azure to AWS (datalakehouse) is underway, which will allow Data Foundation to lean into Tech Enabling over time. No shared decisioning, no EMP.',
        capabilities: [
          { icon: 'partial', text: 'Data warehousing on Azure \u2014 separate from Tech Enabling\u2019s AWS stack' },
          { icon: 'no', text: 'Data Foundation runs own infrastructure \u2014 no Tech Enabling support yet' },
          { icon: 'partial', text: 'Azure \u2192 AWS migration underway \u2014 moving to datalakehouse (Databricks)' },
          { icon: 'no', text: 'No shared CI/CD, observability, or integration platform across domains' },
          { icon: 'no', text: 'No standard way to deploy or monitor AI models in production' },
        ],
        tools: ['Azure DWH', 'Team-specific DBs', 'Ad-hoc notebooks'],
      ...
      {
        desc: 'Product teams experiment with AI independently. No shared building blocks, no domain AI factory pattern. Each team picks their own LLM providers and approaches.',
        capabilities: [
          { icon: 'partial', text: 'Some teams experimenting with AI/LLM integration' },
          { icon: 'no', text: 'No shared building blocks or patterns from a Hub' },
          { icon: 'no', text: 'No standardised approach to building domain AI solutions' },
          { icon: 'no', text: 'No AI governance or risk assessment process' },
          { icon: 'no', text: 'No reusable components \u2014 every product team starts from scratch' },
        ],
  ...
  },
  {
    ...
    personas: [
      ...
      {
        desc: 'Both Hub domains consolidating. Azure \u2192 AWS migration completing \u2014 Data Foundation now runs Databricks on AWS and begins leveraging Tech Enabling\u2019s infrastructure, CI/CD, and observability. Data Foundation focuses on the data layer; Audience domain (outside Hub) begins building the decision layer (EMP + EOL). Tech Enabling: establishing shared dev platform standards and evaluating integration platform.',
        capabilities: [
          { icon: 'yes', text: 'Azure \u2192 AWS migration completing \u2014 Databricks lakehouse on AWS' },
          { icon: 'partial', text: 'Data Foundation starting to use Tech Enabling infrastructure & tooling' },
          { icon: 'partial', text: 'Audience domain: EMP + EOL foundation being built outside Hub' },
          { icon: 'yes', text: 'Tech Enabling: CI/CD pipelines, dev platform standards, observability emerging' },
          { icon: 'yes', text: 'Both: advisory, engineering support, governance framework established' },
        ],
        tools: ['Databricks (AWS)', 'CI/CD', 'Observability (forming)'],
      ...
      {
        desc: 'Building domain-specific data & AI capabilities within product teams. Starting to use shared platform services from the Hub. Initial AI agents and ML models for product-specific use cases.',
        capabilities: [
          { icon: 'yes', text: 'Using Databricks for domain-specific data products and ML models' },
          { icon: 'partial', text: 'Initial AI agents for product-specific use cases in pilot' },
          { icon: 'partial', text: 'Consuming shared Hub services \u2014 data products, governance tooling' },
          { icon: 'yes', text: 'AI risk model and yellow-zone governance for new use cases' },
          { icon: 'partial', text: 'Domain AI factory taking shape \u2014 local agents with local tech choices' },
        ],
  ...
  },
  {
    ...
    personas: [
      ...
      {
        desc: 'Builds data products, AI solutions, and configures personalisation rules on the shared platform. Configures segments, eligibility rules, and audience targeting in the Experience Management Platform (EMP), which is owned by the Audience product team.',
        capabilities: [
          { icon: 'yes', text: 'Databricks workspace for building data products & ML models' },
          { icon: 'yes', text: 'Configures personalisation rules, segments, and journey decisioning in EMP' },
        ...
        tools: ['Databricks', 'AI Platform', 'EMP', 'CoPilot'],
        spoke: 'Spoke \u201cmellem\u201d (medium) \u2014 builds solutions & configures EMP rules',
        maturity: { access: 3, governance: 2.5, ai: 3, value: 2.5 },
        tempo: 'analytics',
      },
      {
        desc: 'Builds channel applications (Play, tv2.dk, Sport, Vejr). Consumes the engagement layer delivered by the Audience product team \u2014 personalised content, offers, and experiences are composed upstream in EMP + EOL. Product-team IT focuses on channel-specific rendering, UX, and streaming/app concerns.',
        capabilities: [
          { icon: 'yes', text: 'Shared datasets and a unified Single Customer View (SCV) via standard interfaces' },
          { icon: 'yes', text: 'Consumes engagement layer from Audience \u2014 does not build decisioning or orchestration' },
          { icon: 'yes', text: 'Builds channel apps: Play, tv2.dk, Sport, Vejr \u2014 rendering, UX, streaming-specific logic' },
          { icon: 'yes', text: 'Common data model ensures the same definitions across all channels' },
          { icon: 'yes', text: 'Channel-unique specializations (e.g. streaming) remain independent' },
        ],
        tools: ['Audience APIs', 'Engagement Layer SDKs', 'Event Streaming', 'Content API'],
        spoke: 'Spoke \u201clille/mellem\u201d \u2014 builds channel apps, consumes Audience\u2019s engagement layer',
        maturity: { access: 3, governance: 2.5, ai: 2.5, value: 3 },
        tempo: 'both',
      },
      {
        desc: 'Spans two Hub domains. Data Foundation: owns the data layer \u2014 data platform, data products, ML models, and shared ontology. Tech Enabling: owns dev platforms, infrastructure, CI/CD, observability, security, and integration platform. Decision layer (EMP + EOL) and engagement layer are owned outside Hub by the Audience product team.',
        capabilities: [
          { icon: 'yes', text: 'Data Foundation: Databricks platform, data products, Unity Catalog, common data model' },
          { icon: 'yes', text: 'Data Foundation: ML models, feature store, shared ontology \u2014 feeds the decision layer' },
          { icon: 'yes', text: 'Tech Enabling: dev platforms, CI/CD pipelines, infrastructure, integration platform' },
          { icon: 'yes', text: 'Tech Enabling: observability, security, platform best practices, tooling standards' },
          { icon: 'yes', text: 'Both: advisory, engineering support, training, governance enforcement' },
        ],
        tools: ['Databricks Unity Catalog', 'ML Platform', 'CI/CD', 'Observability'],
        spoke: 'Hub \u2014 Data Foundation (data layer) + Tech Enabling (infra + dev platforms)',
      ...
      {
        desc: 'Builds and operates AI factories within product teams. Each team can have its own AI factory with domain-specific agents and local technology choices. Hub provides shared building blocks and patterns, but does not centralise all AI. The Audience product team owns both the Decision Layer (EMP + EOL) and the Engagement Layer \u2014 personalised experiences are composed and delivered from Audience, consuming data from the Hub.',
        capabilities: [
          { icon: 'yes', text: 'Product teams own their AI factories \u2014 agents, models, and domain logic' },
          { icon: 'yes', text: 'Audience owns the Decision Layer \u2014 builds & operates EMP + EOL (decisioning, orchestration, delivery)' },
          { icon: 'yes', text: 'Audience owns the Engagement Layer \u2014 personalised experiences delivered to channels' },
          { icon: 'yes', text: 'Hub provides shared AI building blocks, templates, and best practices' },
          { icon: 'yes', text: 'AI governance tooling: risk assessment, approval workflows, monitoring' },
        ],
        tools: ['Databricks AI', 'EMP + EOL (Audience)', 'Agent Framework', 'AI Governance Tooling'],
        spoke: 'Product teams \u2014 own AI factories; Audience owns decision + engagement layers',
...
const PHASES_DA = [
  {
    ...
    personas: [
      ...
      {
        desc: 'Ingen f\u00e6lles platform endnu. Data Foundation k\u00f8rer sit datawarehouse p\u00e5 Azure, mens Tech Enablings stak er AWS. Denne cloud-opsplitning betyder at Data Foundation k\u00f8rer egen infrastruktur \u2014 Tech Enabling kan ikke supportere endnu. Migrering fra Azure til AWS (datalakehouse) er i gang, s\u00e5 Data Foundation kan l\u00e6ne sig mere op ad Tech Enabling over tid. Ingen f\u00e6lles decisioning, ingen EMP.',
        capabilities: [
          { icon: 'partial', text: 'Datawarehouse p\u00e5 Azure \u2014 adskilt fra Tech Enablings AWS-stak' },
          { icon: 'no', text: 'Data Foundation k\u00f8rer egen infrastruktur \u2014 ingen Tech Enabling-support endnu' },
          { icon: 'partial', text: 'Azure \u2192 AWS-migrering i gang \u2014 overgang til datalakehouse (Databricks)' },
          { icon: 'no', text: 'Ingen f\u00e6lles CI/CD, observability eller integrationsplatform p\u00e5 tv\u00e6rs' },
      ...
      {
        desc: 'Produktteams eksperimenterer med AI uafh\u00e6ngigt. Ingen f\u00e6lles byggeklodser, intet dom\u00e6ne-AI-fabriksm\u00f8nster. Hvert team v\u00e6lger egne LLM-udbydere og tilgange.',
        capabilities: [
          { icon: 'partial', text: 'Nogle teams eksperimenterer med AI/LLM-integration' },
          { icon: 'no', text: 'Ingen f\u00e6lles byggeklodser eller m\u00f8nstre fra en Hub' },
          { icon: 'no', text: 'Ingen standardiseret tilgang til at bygge dom\u00e6ne-AI-l\u00f8sninger' },
          { icon: 'no', text: 'Ingen AI governance eller risikovurderingsproces' },
          { icon: 'no', text: 'Ingen genbrugelige komponenter \u2014 hvert produktteam starter fra bunden' },
        ],
        tools: ['Ad-hoc LLM APIs', 'Team experiments', 'No shared infra'],
        maturity: { access: 1, governance: 0, ai: 1, value: 0 },
      },
  ...
  },
  {
    ...
    personas: [
      ...
      {
        desc: 'Begge Hub-dom\u00e6ner konsoliderer. Azure \u2192 AWS-migrering f\u00e6rdigg\u00f8res \u2014 Data Foundation k\u00f8rer nu Databricks p\u00e5 AWS og begynder at bruge Tech Enablings infrastruktur, CI/CD og observability. Data Foundation fokuserer p\u00e5 datalaget; Audience-dom\u00e6net (uden for Hub) begynder at bygge beslutningslaget (EMP + EOL). Tech Enabling: etablerer f\u00e6lles dev-platformstandarder og evaluerer integrationsplatform.',
        capabilities: [
          { icon: 'yes', text: 'Azure \u2192 AWS-migrering f\u00e6rdigg\u00f8res \u2014 Databricks lakehouse p\u00e5 AWS' },
          { icon: 'partial', text: 'Data Foundation begynder at bruge Tech Enablings infrastruktur & tooling' },
          { icon: 'partial', text: 'Audience-dom\u00e6net: EMP + EOL-fundament under opbygning uden for Hub' },
          { icon: 'yes', text: 'Tech Enabling: CI/CD-pipelines, dev-platformstandarder, observability tager form' },
          { icon: 'yes', text: 'Begge: r\u00e5dgivning, engineering-support, governance-framework etableret' },
        ],
        tools: ['Databricks (AWS)', 'CI/CD', 'Observability (under opbygning)'],
        maturity: { access: 2.5, governance: 2, ai: 2, value: 1.5 },
        tempo: 'both',
      },
      {
      ...
      {
        desc: 'Bygger dom\u00e6nespecifikke data & AI-kapabiliteter i produktteams. Begynder at bruge f\u00e6lles platformservices fra Hub. Indledende AI-agenter og ML-modeller til produktspecifikke use cases.',
        capabilities: [
          { icon: 'yes', text: 'Bruger Databricks til dom\u00e6nespecifikke dataprodukter og ML-modeller' },
          { icon: 'partial', text: 'Indledende AI-agenter til produktspecifikke use cases i pilot' },
          { icon: 'partial', text: 'Forbruger f\u00e6lles Hub-services \u2014 dataprodukter, governance-v\u00e6rkt\u00f8jer' },
          { icon: 'yes', text: 'AI-risikomodel og gul-zone governance for nye use cases' },
          { icon: 'partial', text: 'Dom\u00e6ne-AI-fabrik tager form \u2014 lokale agenter med lokale teknologivalg' },
        ],
        tools: ['Databricks', 'LLM APIs', 'Agent Framework (pilot)', 'AI Governance'],
        maturity: { access: 2, governance: 2, ai: 2, value: 1.5 },
        tempo: 'both',
  ...
  },
  {
    ...
    personas: [
      ...
      {
        desc: 'Bygger dataprodukter, AI-l\u00f8sninger og konfigurerer personaliseringsregler p\u00e5 den f\u00e6lles platform. Konfigurerer segmenter, eligibility-regler og m\u00e5lgrupperetning i Experience Management Platformen (EMP), som ejes af Audience-produktteamet.',
        capabilities: [
          { icon: 'yes', text: 'Databricks-workspace til at bygge dataprodukter & ML-modeller' },
          { icon: 'yes', text: 'Konfigurerer personaliseringsregler, segmenter og journey decisioning i EMP' },
        ...
        tools: ['Databricks', 'AI Platform', 'EMP', 'CoPilot'],
        spoke: 'Spoke \u201cmellem\u201d \u2014 bygger l\u00f8sninger & konfigurerer EMP-regler',
        maturity: { access: 3, governance: 2.5, ai: 3, value: 2.5 },
        tempo: 'analytics',
      },
      {
        desc: 'Bygger kanalapplikationer (Play, tv2.dk, Sport, Vejr). Forbruger engagement-laget leveret af Audience-produktteamet \u2014 personaliseret indhold, tilbud og oplevelser komponeres opstr\u00f8ms i EMP + EOL. Produktteam-IT fokuserer p\u00e5 kanalspecifik rendering, UX og streaming/app-anliggender.',
        capabilities: [
          { icon: 'yes', text: 'F\u00e6lles datas\u00e6t og samlet Single Customer View (SCV) via standard-interfaces' },
          { icon: 'yes', text: 'Forbruger engagement-laget fra Audience \u2014 bygger ikke decisioning eller orkestration' },
          { icon: 'yes', text: 'Bygger kanalapps: Play, tv2.dk, Sport, Vejr \u2014 rendering, UX, streaming-specifik logik' },
          { icon: 'yes', text: 'F\u00e6lles datamodel sikrer samme definitioner p\u00e5 tv\u00e6rs af alle kanaler' },
          { icon: 'yes', text: 'Kanalunike specialiseringer (f.eks. streaming) forbliver uafh\u00e6ngige' },
        ],
        tools: ['Audience APIs', 'Engagement Layer SDKs', 'Event Streaming', 'Content API'],
        spoke: 'Spoke \u201clille/mellem\u201d \u2014 bygger kanalapps, forbruger Audiences engagement-lag',
        maturity: { access: 3, governance: 2.5, ai: 2.5, value: 3 },
        tempo: 'both',
      },
      {
        desc: 'D\u00e6kker to Hub-dom\u00e6ner. Data Foundation: ejer datalaget \u2014 dataplatformen, dataprodukter, ML-modeller og f\u00e6lles ontologi. Tech Enabling: ejer udviklingsplatforme, infrastruktur, CI/CD, observability, sikkerhed og integrationsplatform. Beslutningslaget (EMP + EOL) og engagement-laget ejes uden for Hub af Audience-produktteamet.',
        capabilities: [
          { icon: 'yes', text: 'Data Foundation: Databricks-platform, dataprodukter, Unity Catalog, f\u00e6lles datamodel' },
          { icon: 'yes', text: 'Data Foundation: ML-modeller, feature store, f\u00e6lles ontologi \u2014 f\u00f8der beslutningslaget' },
          { icon: 'yes', text: 'Tech Enabling: udviklingsplatforme, CI/CD-pipelines, infrastruktur, integrationsplatform' },
          { icon: 'yes', text: 'Tech Enabling: observability, sikkerhed, platform best practices, tooling-standarder' },
          { icon: 'yes', text: 'Begge: r\u00e5dgivning, engineering-support, tr\u00e6ning, governance-h\u00e5ndh\u00e6velse' },
        ],
        tools: ['Databricks Unity Catalog', 'ML Platform', 'CI/CD', 'Observability'],
        spoke: 'Hub \u2014 Data Foundation (datalaget) + Tech Enabling (infra + udviklingsplatforme)',
        maturity: { access: 3.5, governance: 3, ai: 3.5, value: 2.5 },
        tempo: 'both',
      },
      {
      ...
      {
        desc: 'Bygger og driver AI-fabrikker inden for produktteams. Hvert team kan have sin egen AI-fabrik med dom\u00e6nespecifikke agenter og lokale teknologivalg. Hub leverer f\u00e6lles byggeklodser og m\u00f8nstre, men centraliserer ikke al AI. Audience-produktteamet ejer b\u00e5de beslutningslaget (EMP + EOL) og engagement-laget \u2014 personaliserede oplevelser komponeres og leveres fra Audience, der forbruger data fra Hub.',
        capabilities: [
          { icon: 'yes', text: 'Produktteams ejer deres AI-fabrikker \u2014 agenter, modeller og dom\u00e6nelogik' },
          { icon: 'yes', text: 'Audience ejer beslutningslaget \u2014 bygger & driver EMP + EOL (decisioning, orkestration, levering)' },
          { icon: 'yes', text: 'Audience ejer engagement-laget \u2014 personaliserede oplevelser leveret til kanaler' },
          { icon: 'yes', text: 'Hub leverer f\u00e6lles AI-byggeklodser, skabeloner og best practices' },
          { icon: 'yes', text: 'AI governance-v\u00e6rkt\u00f8jer: risikovurdering, godkendelsesworkflows, monitorering' },
        ],
        tools: ['Databricks AI', 'EMP + EOL (Audience)', 'Agent Framework', 'AI Governance Tooling'],
        spoke: 'Produktteams \u2014 ejer AI-fabrikker; Audience ejer beslutnings- + engagement-lag',
        maturity: { access: 3.5, governance: 3, ai: 3.5, value: 3 },
        tempo: 'operational',
      },
    ],
...
const GLOSSARY_EN = [
  ...
  { term: 'Experience Management Platform', short: 'EMP \u2014 centralised decisioning for personalisation', detail: 'The Decision layer in TV 2\u2019s three-layer architecture (Data \u2192 Decision \u2192 Engagement). Houses content recommendations, customer journey decisioning, real-time CDP activation, segmentation, and campaign management. Owned by the Audience product team (outside Hub).' },
  { term: 'Experience Orchestration Layer', short: 'EOL \u2014 headless omnichannel delivery', detail: 'A centralised runtime composition and orchestration capability for personalised, headless omnichannel content delivery. Combines channel and customer context with recommendations from the EMP. Owned by the Audience product team (outside Hub) \u2014 sits alongside EMP in the Decision layer.' },
...
const GLOSSARY_DA = [
  ...
  { term: 'Experience Management Platform', short: 'EMP \u2014 centraliseret beslutningstagning for personalisering', detail: 'Beslutningslaget i TV 2\u2019s tre-lags m\u00e5larkitektur (Data \u2192 Beslutning \u2192 Engagement). Rummer indholdsanbefalinger, kundrejse-beslutningstagning, realtids CDP-aktivering, segmentering og kampagnestyring. Ejes af Audience-produktteamet (uden for Hub).' },
  { term: 'Experience Orchestration Layer', short: 'EOL \u2014 headless omnichannel-levering', detail: 'En centraliseret runtime-komposition og orkestreringskapacitet til personaliseret, headless omnichannel-indholdslevering. Kombinerer kanal- og kundekontekst med anbefalinger fra EMP. Ejes af Audience-produktteamet (uden for Hub) \u2014 sidder ved siden af EMP i beslutningslaget.' },
...
const UI_EN = {
  ...
  govRingTop: 'GOVERNANCE \u00B7 COMMON DATA MODEL \u00B7 DESIGN AUTHORITY (EA)',
  govRingBottom: 'DATA LAYER \u00B7 DECISION LAYER (EMP + EOL) \u00B7 ENGAGEMENT LAYER',
...
const UI_DA = {
  ...
  hubFull: 'Platform \u00B7 Governance \u00B7 AI',
  hubExtra: 'Self-Service \u00B7 Enablement',
  govRingTop: 'GOVERNANCE \u00B7 F\u00c6LLES DATAMODEL \u00B7 DESIGN AUTHORITY (EA)',
  govRingBottom: 'DATALAG \u00B7 BESLUTNINGSLAG (EMP + EOL) \u00B7 ENGAGEMENTLAG',
...
const PLATFORM_PAGE_EN = {
  ...
  subtitle: 'Two distinct platforms owned by two domains within the Hub. The Data domain owns the data platform (storage, transformation, serving). The Tech Enabling domain owns the integration platform and developer infrastructure. Enterprise Architecture sets principles across both. Other domains are consumers and producers.',

  // Domain 4: Enterprise Architecture (top banner)
  eaDomainTitle: 'ENTERPRISE ARCHITECTURE',
  eaDomainDesc: 'Architecture principles · Data governance · Security & privacy',
  eaDomainLabel: 'Domain 4 — sets standards across all platforms',

  // Domain 2: Data (left side)
  dataDomainTitle: 'Data Platform',
  dataDomainTech: 'Databricks · lakehouse architecture',
  dataDomainLabel: 'DOMAIN 2 — DATA (DATA FOUNDATION)',

  hubTitle: 'Hub — Data Foundation',
  hubDesc: 'Master data, ontology, common data models, data products',
  hubOwner: 'Owner: Data domain (Data Foundation)',
  ...
  analyticsConsumers: 'All domains consume analytics',
  ...
  techDomainTitle: 'Tech Enabling Platform',
  techDomainDesc: 'Infrastructure · integration · developer platforms',
  techDomainLabel: 'DOMAIN 1 — TECH ENABLING',
  ...
  devPlatTitle: 'Developer Platforms',
  devPlatDesc: 'CI/CD · container platforms · IaC',
  devPlatTech: 'GitHub · ArgoCD · Kubernetes',
  devPlatTech2: 'Shared toolchains & golden paths',

  obsSecTitle: 'Observability & Security',
  obsSecDesc: 'Monitoring · logging · alerting · IAM',
  obsSecTech: 'Datadog / Grafana · Vault · SSO',
  obsSecTech2: 'Cross-platform visibility & compliance',

  // Domain 3: Other Domains (consumers in both platforms)
  otherDomainsTitle: 'Other Domains (Data Consumers & Producers)',
  otherDomainsDesc: 'Build and own domain data products following hub standards',
  otherDomainsExamples: ['Content & Editorial', 'Ads & Revenue', 'Viewer & Subs'],

  otherDomainsIntTitle: 'Other Domains (Integration Consumers)',
  otherDomainsIntDesc: 'Use integration platform to connect systems and exchange data',
  otherDomainsIntExamples: 'TV 2 Play · News · Sport · Ads · Subscriptions',

  // Arrows
  flowLabelOut: 'data out',
  flowLabelIn: 'data in',
  ...
  // Legend
  legendTitle: 'DOMAINS',
  legendData: 'Data (Data Foundation)',
  legendTech: 'Tech Enabling',
  legendOther: 'Other Domains',
  legendEA: 'Enterprise Architecture',

  cards: [
    { icon: '\uD83C\uDFDB\uFE0F', color: '#10b981', title: 'Data domain owns the data platform', desc: 'Master data, common ontology, data products catalog, medallion architecture, and platform standards. Spokes (other domains) own their domain data products within this platform.' },
    { icon: '\uD83D\uDD27', color: '#3b82f6', title: 'Tech Enabling owns infrastructure', desc: 'Integration platform (API gateway, event bus), developer platforms (CI/CD, containers), observability, and security. Integration tech stack is under active evaluation.' },
    { icon: '\uD83D\uDCD0', color: '#8b5cf6', title: 'Enterprise Architecture sets standards', desc: 'Architecture principles, data governance framework, security and privacy requirements apply across both platforms and all domains.' },
    { icon: '\u26A1', color: '#f59e0b', title: 'Two tempos, one data platform', desc: 'The data platform serves both analytics (batch, dashboards) and operational (streaming, real-time personalisation) needs from a single lakehouse.' },
    { icon: '\uD83D\uDD17', color: '#ec4899', title: 'Other domains consume & produce', desc: 'Non-platform domains (Content, Ads, Viewer, etc.) are both consumers and producers. They own their domain data products and use shared integration to connect systems.' },
...
const PLATFORM_PAGE_DA = {
  ...
  subtitle: 'To distinkte platforme ejet af to domæner i Hub. Data-domænet ejer dataplatformen (lagring, transformation, servering). Tech Enabling-domænet ejer integrationsplatformen og udviklerinfrastruktur. Enterprise Architecture sætter principper på tværs. Andre domæner er forbrugere og producenter.',

  // Domain 4: Enterprise Architecture (top banner)
  eaDomainTitle: 'ENTERPRISE ARCHITECTURE',
  eaDomainDesc: 'Arkitekturprincipper · Data governance · Sikkerhed & privatliv',
  eaDomainLabel: 'Domæne 4 — sætter standarder på tværs af alle platforme',

  // Domain 2: Data (left side)
  dataDomainTitle: 'Dataplatform',
  dataDomainTech: 'Databricks · lakehouse-arkitektur',
  dataDomainLabel: 'DOMÆNE 2 — DATA (DATA FOUNDATION)',

  hubTitle: 'Hub — Data Foundation',
  hubDesc: 'Stamdata, ontologi, fælles datamodeller, dataprodukter',
  hubOwner: 'Ejer: Data-domænet (Data Foundation)',
  hubTech: 'Databricks Unity Catalog · Delta Lake · dbt',
  ...
  analyticsConsumers: 'Alle domæner bruger analytics',
  ...
  // Domain 1: Tech Enabling (right side)
  techDomainTitle: 'Tech Enabling Platform',
  techDomainDesc: 'Infrastruktur · integration · udviklerplatforme',
  techDomainLabel: 'DOMÆNE 1 — TECH ENABLING',
  ...
  devPlatTitle: 'Udviklerplatforme',
  devPlatDesc: 'CI/CD · containerplatforme · IaC',
  devPlatTech: 'GitHub · ArgoCD · Kubernetes',
  devPlatTech2: 'Delte toolchains & golden paths',

  obsSecTitle: 'Observability & Sikkerhed',
  obsSecDesc: 'Monitoring · logging · alerting · IAM',
  obsSecTech: 'Datadog / Grafana · Vault · SSO',
  obsSecTech2: 'Synlighed & compliance på tværs',

  // Domain 3: Other Domains (consumers in both platforms)
  otherDomainsTitle: 'Andre domæner (Dataforbrugere & -producenter)',
  otherDomainsDesc: 'Bygger og ejer domæne-dataprodukter efter hub-standarder',
  otherDomainsExamples: ['Content & Redaktion', 'Ads & Revenue', 'Seer & Abonnement'],

  otherDomainsIntTitle: 'Andre domæner (Integrationsforbrugere)',
  otherDomainsIntDesc: 'Bruger integrationsplatformen til at forbinde systemer og udveksle data',
  otherDomainsIntExamples: 'TV 2 Play · Nyheder · Sport · Annoncer · Abonnementer',

  // Arrows
  flowLabelOut: 'data ud',
  flowLabelIn: 'data ind',
  ...
  legendData: 'Data (Data Foundation)',
  legendTech: 'Tech Enabling',
  legendOther: 'Andre domæner',
  legendEA: 'Enterprise Architecture',

  cards: [
    { icon: '\uD83C\uDFDB\uFE0F', color: '#10b981', title: 'Data-domænet ejer dataplatformen', desc: 'Stamdata, fælles ontologi, dataproduktkatalog, medallion-arkitektur og platformstandarder. Spokes (andre domæner) ejer deres domæne-dataprodukter inden for denne platform.' },
    { icon: '\uD83D\uDD27', color: '#3b82f6', title: 'Tech Enabling ejer infrastruktur', desc: 'Integrationsplatform (API gateway, event bus), udviklerplatforme (CI/CD, containere), observability og sikkerhed. Integrationsteknologi er under aktiv evaluering.' },
    { icon: '\uD83D\uDCD0', color: '#8b5cf6', title: 'Enterprise Architecture sætter standarder', desc: 'Arkitekturprincipper, data governance-framework, sikkerheds- og privatlivskrav gælder på tværs af begge platforme og alle domæner.' },
    { icon: '\u26A1', color: '#f59e0b', title: 'To tempi, én dataplatform', desc: 'Dataplatformen betjener både analytics (batch, dashboards) og operationelle (streaming, realtidspersonalisering) behov fra ét samlet lakehouse.' },
    { icon: '\uD83D\uDD17', color: '#ec4899', title: 'Andre domæner forbruger & producerer', desc: 'Ikke-platform-domæner (Content, Ads, Seer osv.) er både forbrugere og producenter. De ejer deres domæne-dataprodukter og bruger delt integration til at forbinde systemer.' },
